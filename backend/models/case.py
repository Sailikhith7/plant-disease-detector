...
# One-time migration for pre-existing databases: add the audio_url column to
# expert_responses if it isn't there yet. Checked via the inspector instead of
# "ALTER TABLE ... ADD COLUMN IF NOT EXISTS" because that clause isn't valid
# SQLite syntax (it silently fails there) even though it works on Postgres.
try:
...
def generate_tts_audio(text_content: str, lang: str = "mr") -> str:
    """Generate (or reuse a cached) voice-prescription MP3 for an expert's
    written response, so farmers can hear it read aloud in their language."""
    try:
