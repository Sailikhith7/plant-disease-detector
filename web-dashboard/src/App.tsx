...
function App() {
  ...
  const fetchCases = useCallback(async () => {
    try {
      ...
      const formattedCases: MockCase[] = backendCases.map((item: any) => {
        let confidence = 0;
        ...
        return {
          ...
          // GPS coordinates
          gps_lat: item.gps_lat ?? item.lat ?? item.latitude ?? 20.3888,
          gps_long: item.gps_long ?? item.lng ?? item.longitude ?? 78.1204,
          ...
          // Resolution date
          resolution_date: item.resolved_at
            ? new Date(item.resolved_at).toISOString().split("T")[0]
            : undefined,

          // Expert diagnosis
          expert_diagnosis:
            item.expert_diagnosis || item.expert_response || undefined,
  ...
  // =========================================
  // PENDING CASES (not yet resolved, any confidence —
  // ExpertTriagePage splits these into "Pending Expert
  // Review" vs "AI Auto-Resolved" by confidence itself)
  // =========================================
  ...
  // =========================================
  // BULK APPROVE (AI Auto-Resolved -> Resolved)
  //
  // Ported from the web-dashboard branch's "Approve All"
  // feature. There's no separate "Approved for Analytics"
  // status in the backend, so this just calls the existing
  // resolve endpoint for each case ID with a standard
  // auto-approval note, then refreshes.
  // =========================================
  const handleApproveAll = async (caseIds: (string | number)[]) => {
    if (caseIds.length === 0) {
      return;
    }
    ...
    try {
      await Promise.all(
        caseIds.map((caseId) =>
          fetch(`${API_BASE_URL}/cases/${caseId}/resolve`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              expert_response: "Auto-approved (AI confidence >= threshold).",
            }),
          })
        )
      );
      ...
      console.error("[PikRakshak] Bulk approve error:", error);
    }
  };
